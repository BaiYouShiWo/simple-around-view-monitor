...
parser.add_argument('-input', '--INPUT_TYPE', default='camera', type=str, help='Input Source: camera/video/image')#default dcamera but i change it into img
parser.add_argument('-type', '--CAMERA_TYPE', default='fisheye', type=str, help='Camera Type: fisheye/normal')
parser.add_argument('-id', '--CAMERA_ID', default=6, type=int, help='Camera ID')
...
parser.add_argument('-image', '--IMAGE_FILE', default='img_src', type=str, help='Input Image File Name Prefix (eg.: img_raw)')
parser.add_argument('-mode', '--SELECT_MODE', default='auto', type=str, help='Image Select Mode: auto/manual')
parser.add_argument('-fw','--FRAME_WIDTH', default=640, type=int, help='Camera Frame Width')
parser.add_argument('-fh','--FRAME_HEIGHT', default=480, type=int, help='Camera Frame Height')
parser.add_argument('-bw','--BORAD_WIDTH', default=12, type=int, help='Chess Board Width (corners number)')
parser.add_argument('-bh','--BORAD_HEIGHT', default=8, type=int, help='Chess Board Height (corners number)')
parser.add_argument('-size','--SQUARE_SIZE', default=20, type=int, help='Chess Board Square Size (mm)')
...
parser.add_argument('-fps','--CAMERA_FPS', default=30, type=int, help='Camera Frame per Second(FPS)')
parser.add_argument('-fs', '--FOCAL_SCALE', default=2, type=float, help='Camera Undistort Focal Scale')
...
parser.add_argument('-store_path', '--STORE_PATH', default='./data/temp/', type=str, help='Path to Store Captured Images')
...
def main():
    ...
    np.save('480p_camera_{}_K.npy'.format(name),result.camera_mat.tolist())#args.CAMERA_ID
    np.save('480p_camera_{}_D.npy'.format(name),result.dist_coeff.tolist())#args.CAMERA_ID
